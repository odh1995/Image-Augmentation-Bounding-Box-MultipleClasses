...
def dataAug(infile):
    ...
    name, fext = os.path.splitext(path + '/' + filename)
    justName, fType = os.path.splitext(filename)
    annotfile = (name + ".txt")
    print (">> " + annotfile)
    try:
    ...
    for idx,image in enumerate(images):
        ...
        outfile = open(path + "/" + str(idx) + '-' + justName + '.txt', 'w')
        ...
        if os.path.getsize(outfile.name) > 0:
            imageio.imwrite(path + "/" + str(idx) + '-' + filename, image_aug)  #write all changed images
        else:
